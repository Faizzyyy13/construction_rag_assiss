...
2. **Install dependencies:**
   ...
   *(Note: ensure your requirements.txt includes: streamlit, sentence-transformers, faiss-cpu, requests, numpy, pdfplumber)*

3. **Add your documents:**
   Place the required PDF documents (`doc1.pdf`, `doc2.pdf`, `doc3.pdf`) in the root directory of the project.
...
**Model Used:** OpenRouter Dynamic Free Router (`openrouter/free`) / Google Gemma 2 9B
**Why:** OpenRouter provides a seamless API to access powerful, open-weights models for free. Using a highly capable instruct-tuned model ensures the generated text is coherent and strictly adheres to the system prompts.
...
##  Transparency
The user interface is designed for explainability. Every time an answer is generated, the frontend explicitly displays the exact retrieved context chunks that the LLM used to formulate its response.

##  Quality Analysis 

To evaluate the system's performance, a test suite of 12 questions was created based on the internal Indecimal documents. These questions test factual retrieval, synthesis, and the system's ability to refuse unanswerable questions.
...
**Synthesis & Process**
6. What operational mechanisms does Indecimal use to enforce its Zero-Tolerance Policy on Construction Delays?
7. What are the 10 stages of the customer journey?
8. How does Indecimal differentiate itself from typical market alternatives?
...
### Evaluation Findings
* **Relevance of Retrieved Chunks:** The `all-MiniLM-L6-v2` embedding model combined with FAISS performs exceptionally well. For highly specific queries (e.g., "escrow-based payment"), the system consistently ranks the exact relevant document chunk as the top result.
* **Presence of Hallucinations:** The prompt engineering successfully constrains the LLM. When asked the boundary test questions (Questions 10-12) about cement brands or commercial spaces, the model correctly outputs that it does not have enough information, rather than fabricating plausible-sounding general construction facts.
* **Completeness and Clarity:** The LLM effectively translates the raw, bulleted markdown from the retrieved chunks into natural, easy-to-read conversational answers without losing critical numerical details (like the 445+ checkpoints or the ~30-day disbursal timeline).
